using CrmSdkLibrary.Dataverse.Definition;
using CrmSdkLibrary.Dataverse.Definition.Enum;
using CrmSdkLibrary.Dataverse.Definition.StaticFiles;
...
namespace CrmSdkLibrary.Dataverse
{
    public static partial class Messages
    {
        ...
        /// </code>
        /// </example>
        public static Microsoft.Crm.Sdk.Messages.AccessRights RetrievePrincipalAccess(in IOrganizationService service, EntityReference target, EntityReference principal) => (service.Execute(new RetrievePrincipalAccessRequest
        {
            Principal = principal,
            Target = target
        ...
        /// </code>
        /// </example>
        public static string RetrievePrincipalAccessInfo(in IOrganizationService service, EntityReference target, EntityReference principal) => (service.Execute(new RetrievePrincipalAccessInfoRequest
        {
            Principal = principal,
        ...
        public static IEnumerable<Entity> RetrieveChildRecordsWithReadAccess(IOrganizationService service, string childEntityName, string parentLookupFieldName, Guid parentId, params string[] columns)
        {
            ...
            var qe = new QueryExpression(childEntityName)
            {
                ColumnSet = columns.Length == 0 ? new ColumnSet(true) : new ColumnSet(columns),
                LinkEntities =
                {
                    new LinkEntity(childEntityName, "principalobjectaccess",childEntityName + "id","objectid", JoinOperator.LeftOuter)
                    {
                        //LinkCriteria = new FilterExpression //(principaltypecode) 팀이 있어서 불가
                        //{
                        //    Conditions =
                        //    {
            ...
            return service.RetrieveMultiple(qe).Entities.Where(e => ((AccessRights)e.GetAttributeValue<int>("accessrightsmask") & AccessRights.ReadAccess) == AccessRights.ReadAccess).GroupBy(x => x.Id).Select(x => x.FirstOrDefault());
        }
      ...
      //                  }
            ...
            return service.RetrieveMultiple(qe).Entities
                .Select(e => (
                    Id: e.Id,
                    Rights: (AccessRights)e.GetAttributeValue<int>("accessrightsmask")
                ));
        }


        /// <summary>
        /// 특정 레코드에 대한 모든 접근 권한 정보(사용자와 팀)를 조회합니다.
        /// Retrieves all access right information (users and teams) for a specific record.
        /// </summary>
        /// <param name="service">CRM 조직 서비스 / The organization service</param>
        /// <param name="record">접근 권한을 조회할 레코드 참조 / Record reference to check access rights</param>
        /// <returns>
        /// 접근 권한을 가진 사용자/팀 정보와 권한 목록을 튜플로 반환:
        /// - EntityLogicalName: 엔터티 논리명(systemuser/team)
        /// - PrincipalId: 사용자/팀의 ID
        /// - Name: 사용자 전체이름 또는 팀 이름
        /// - AccessRights: 접근 권한
        /// 
        /// Returns tuple collection containing:
        /// - EntityLogicalName: Logical name of entity (systemuser/team)
        /// - PrincipalId: User/Team ID
        /// - Name: User's full name or Team name
        /// - AccessRights: Access rights
        /// </returns>
        /// <example>
        /// <code>
        /// // 계정 레코드에 대한 접근 권한 조회
        /// // Check access rights for an account record
        /// var record = new EntityReference("account", accountId);
        /// var accessList = RetrieveRecordAccessList(service, record);
        /// 
        /// foreach (var (entityType, principalId, name, accessRights) in accessList)
        /// {
        ///     Console.WriteLine($"{entityType} {name} has accessRights: {accessRights}");
        /// }
        /// </code>
        /// </example>
        public static IEnumerable<(string EntityLogicalName, Guid PrincipalId, string Name, AccessRights AccessRights)>  RetrieveRecordAccessList(IOrganizationService service, EntityReference record)
        {
            var query = new QueryExpression("principalobjectaccess")
            {
                ...
                Criteria = new FilterExpression
                {
                    Conditions =
                    {
                        new ConditionExpression("objecttypecode", ConditionOperator.Equal, record.LogicalName),
                        new ConditionExpression("objectid", ConditionOperator.Equal, record.Id)
                    }
                },
                LinkEntities =
                {
                    new LinkEntity("principalobjectaccess", "systemuser", "principalid", "systemuserid", JoinOperator.LeftOuter)
                    {
                        EntityAlias = "user",
                        Columns = new ColumnSet("fullname")
                    },
                    new LinkEntity("principalobjectaccess", "team", "principalid", "teamid", JoinOperator.LeftOuter)
                    {
                        EntityAlias = "team",
            ...
            return service.RetrieveMultiple(query).Entities.Select(e =>
            {
                var principalType = e.GetAttributeValue<string>("principaltypecode");

                return (
                    EntityLogicalName: principalType,
                    PrincipalId: e.GetAttributeValue<Guid>("principalid"),
                    Name: principalType == "systemuser"
                        ? (e.GetAttributeValue<AliasedValue>("user.fullname")?.Value?.ToString() ?? "")
                        : (e.GetAttributeValue<AliasedValue>("team.name")?.Value?.ToString() ?? ""),
                    AccessRights: (AccessRights)e.GetAttributeValue<int>("accessrightsmask")
                );
            });
        }
        ...
        /// 
        /// </summary>
        /// <see href="https://learn.microsoft.com/en-us/dotnet/api/microsoft.crm.sdk.messages.exportmappingsimportmaprequest?view=dataverse-sdk-latest"/>
        /// <param name="service"></param>
        /// <param name="importMapId">import map (data map) guid to export</param>
        /// <param name="exportIds"></param>
        /// <returns></returns>
        public static string ExportMappingsImportMap(in IOrganizationService service, Guid importMapId, bool exportIds = false)
        {
            return (service.Execute(new ExportMappingsImportMapRequest
            {
        ...
        /// 
        /// </summary>
        /// <see href="https://learn.microsoft.com/en-us/dotnet/api/microsoft.crm.sdk.messages.importmappingsimportmaprequest?view=dataverse-sdk-latest"/>
        /// <param name="service"></param>
        /// <param name="mappingsXml"></param>
        /// <param name="replaceIds"></param>
        /// <returns>newly created import map (data map) guid</returns>
        public static Guid ImportMappingsImportMap(in IOrganizationService service, string mappingsXml, bool replaceIds = false)
        {
            return (service.Execute(new ImportMappingsImportMapRequest
            {
                MappingsXml = mappingsXml,
                ReplaceIds = replaceIds,
            }) as ImportMappingsImportMapResponse).ImportMapId;
        }
    }
}